...
// newMemorySubscription 创建新订阅
func newMemorySubscription(topic string, handler EventHandler, config *SubscribeConfig) *memorySubscription {
...
// start 启动订阅处理
func (s *memorySubscription) start(ctx context.Context, pool PoolSubmitter) {
	// 启动事件处理协程
...
// processEvents 处理事件
func (s *memorySubscription) processEvents(ctx context.Context) {
	for {
		select {
...
// handleEvent 处理单个事件
func (s *memorySubscription) handleEvent(ctx context.Context, event *Event) {
	var err error
...
// add 添加订阅
func (sm *subscriberManager) add(sub *memorySubscription) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
// remove 移除订阅
func (sm *subscriberManager) remove(sub *memorySubscription) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
func (sm *subscriberManager) getSubscribers(topic string) []*memorySubscription {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	...
	// 返回副本，避免并发问题
	result := make([]*memorySubscription, len(subs))
	copy(result, subs)
	return result
}

// getAllSubscribers 获取所有订阅者
func (sm *subscriberManager) getAllSubscribers() []*memorySubscription {
	sm.mu.RLock()
	defer sm.mu.RUnlock()

	var result []*memorySubscription
